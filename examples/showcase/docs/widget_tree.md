Hierarchical expandable node display with virtualization, lazy-loading, keyboard
navigation, and drag-reorder support.
...
`TreeNodeCfg.ID` defaults to `Text` when omitted. Node IDs must be unique within
a tree.
...
`Up` / `Down` / `Left` (collapse) / `Right` (expand) / `Home` / `End` / `Enter`
/ `Space`

## Stdlib Data Binding

Use `ItemPaths []string` for flat path strings. Slash-separated paths
auto-expand into nested nodes:
