...
Go-Gui is a declarative GUI framework for Go focused on building desktop
interfaces with clear, composable code. Views are described with rows, columns,
and controls, while the framework handles layout, painting, focus, and event
routing.
...
- Accessibility and productivity features: focus ids, keyboard nav, IME-aware
  input

## 🧭 About Showcase

This program is an interactive component catalog. Use the left panel to filter
categories and components, then inspect live demos on the right. Each demo
includes related example files for deeper reference.
