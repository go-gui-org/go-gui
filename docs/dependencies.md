...
[`go.mod`](../go.mod) (declarations) and [`go.sum`](../go.sum) (content
checksums).
...
`go.mod` on `main` must not carry a local replace — CI fetches the tagged
module.

## Verification

Pre-commit checks (PostToolUse hook runs lint-fix + tests on every `.go` edit;
